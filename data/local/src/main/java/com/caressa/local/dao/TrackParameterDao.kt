...
//    abstract suspend fun getParameterDashboardData():List<TrackParameterMaster.ParamDashboardData>
    ...
    abstract suspend fun getParameterHisBaseOnCode(pCode: String, personId: String): List<TrackParameterMaster.History>

    @Query("SELECT * from TrackParameterHistory where profileCode =:pCode AND personID =:personId ORDER by recordDate DESC")
    abstract suspend fun getParameterHisBaseOnProfileCode(pCode: String, personId: String): List<TrackParameterMaster.History>
    ...
    abstract suspend fun getParameterBasedOnProfileCodeAndDate(pCode: String,personId: String,limit:Int): List<TrackParameterMaster.History>
    ...
    abstract suspend fun getParameterObservationBasedOnParameterCode(pCode:String, age: Int,gender: Int): List<TrackParameterMaster.TrackParameterRanges>
...
//    @Query("SELECT personID,parameterCode,description,profileCode,value,observation,recordDate,max(recordDateMillisec) as recordDateMillisec,textValue,ownerCode,sync,unit,createdBy,createdDate,modifiedBy FROM TrackParameterHistory WHERE personID =:personId  group by parameterCode HAVING ProfileCode =:pCode")
    ...
    abstract suspend fun getParameterCodeHistory(paramCode: String, personId: String):List<TrackParameterMaster.History>
    ...
    abstract suspend fun listHistoryWithLatestRecord(personId: String,profileCode: String): List<TrackParameterMaster.History>
        ...
        try {
            ...
            try {
                for (item in data.parameters) {
                    parmItem = TrackParameterMaster.Parameter(iD = item.iD,code = item.code,description = item.description,hasUnit = item.hasUnit,isMandatory = item.isMandatory,minPermissibleValue = item.minPermissibleValue,
                        maxPermissibleValue = item.maxPermissibleValue,comments = "", profileCode = item.profileCode, profileName = item.profileName, unit = item.unit, parameterType = item.parameterType, isRecordExist = item.isRecordExist)
//            Timber.i("Param=> "+parmItem)
        ...
        try {
            ...
            for (item in data.bMIHistory){
                if( !Utilities.isNullOrEmptyOrZero(item.height.toString()) ) {
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BMI", value = item.height, parameterCode = "HEIGHT",
                        description = "HEIGHT", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "cm", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if ( !Utilities.isNullOrEmptyOrZero(item.weight.toString()) ) {
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BMI", value = item.weight, parameterCode = "WEIGHT",
                        description = "WEIGHT", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "Kg", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if (!Utilities.isNullOrEmptyOrZero(item.value)) {
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BMI", value = item.value.toDouble(), parameterCode = "BMI",
                        description = "BMI",observation = item.observation, recordDateMillisec = item.recordDateMillisec ,recordDate = item.recordDate, unit = "kg/m2", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
            }
    ...
    suspend fun saveWHRHistory(data: WHRHistoryModel.Response){
        try {
            var bmiItem: TrackParameterMaster.History
            val listHistory: ArrayList<TrackParameterMaster.History> = arrayListOf<TrackParameterMaster.History>()
            for (item in data.wHRHistory){
                if(item.waist != 0.0){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "WHR", value = item.waist, parameterCode = "WAIST",
                        description = "WAIST", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "cm", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if (item.hip != 0.0){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "WHR", value = item.hip, parameterCode = "HIP",
                        description = "HIP", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "cm", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if (!item.value.equals("",true)){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "WHR", value = item.value.toDouble(), parameterCode = "WHR",
                        description = "WHR",observation = item.observation, recordDateMillisec = item.recordDateMillisec ,recordDate = item.recordDate, unit = "", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
            }
            insertHistory(listHistory)
    ...
    suspend fun saveBloodPressureHistory(data: BloodPressureHistoryModel.Response){
        try {
            ...
            for (item in data.bloodPressureHistory){
                if(item.systolic != 0){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BLOODPRESSURE", value = item.systolic.toDouble(), parameterCode = "BP_SYS",
                        description = "Systolic", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "mmHg", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if (item.diastolic != 0){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BLOODPRESSURE", value = item.diastolic.toDouble(), parameterCode = "BP_DIA",
                        description = "Diastolic", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "mmHg", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
                if (item.pulse != 0){
                    bmiItem = TrackParameterMaster.History(personID = item.personID, profileCode = "BLOODPRESSURE", value = item.pulse.toDouble(), parameterCode = "BP_PULSE",
                        description = "Pulse", recordDate = item.recordDate, recordDateMillisec = item.recordDateMillisec ,unit = "bpm", sync = false, createdDate = item.createdDate, modifiedDate = item.modifiedDate)
                    listHistory.add(bmiItem)
                }
            }
            insertHistory(listHistory)
