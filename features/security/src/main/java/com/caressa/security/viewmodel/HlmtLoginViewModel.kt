...
class HlmtLoginViewModel(private val userManagementUseCase: UserManagementUseCase, private val dispatchers: AppDispatchers,
    ...
    fun fetchLoginResponse(name: String = "", username: String, passwordStr: String = "") = viewModelScope.launch(dispatchers.main){
        ...
        val requestData = LoginModel(Gson().toJson(LoginModel.JSONDataRequest(
    ...
    private fun saveUserData(usr: LoginModel.Data)= viewModelScope.launch {
        var user = Users(accountId = usr.accountID,personId = 1,firstName = usr.name,dateOfBirth = "21/11/1989",gender = "1",age = 32
    ...
    object LoginEncryption {
        fun getHlmtEncryptedData(mode: String,
            ...
            registerObject.put("PartnerCode", "HLMT")
        ...
        _isPhone.addSource(phoneSource) {
            ...
            if (it.status == Resource.Status.SUCCESS) {
                ...
                if (it.data?.isExist.equals("false", true)) {
                    ...
                    callLogin(forceRefresh = true,mobileStr = it.data!!.account!!.emailAddress.toString(), passwordStr = passwordStr)
                }
            }
